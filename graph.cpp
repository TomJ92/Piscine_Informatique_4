...
std::map<int,std::vector<int>> Graph::algo_forte_connexite()
{
    ...
    int i=0;
    bool* marques=new bool[m_vertices.size()]; ///Déclaration du tableau de booléens qui dit si un sommet a été traité ou pas
    bool** compos_connexes; ///Tableau qui détermine les composantes fortement connexes
    compos_connexes=new bool*[m_vertices.size()]; ///1ere allocation de mémoire
    for(int i=0; i<m_vertices.size(); i++) ///2ème allocation de mémoire
    ...
    {
        if(!el.second.m_marque) ///Si il n'a pas été traité
        {
            ...
            {
                if(compos_connexes[el.first][j] && !marques[j]) ///Si i et j font partie d'une composante fortement connexe et que j n'a pas été traité
                {
                    el.second.m_marque=true; ///On le marque
                }
            }
    ...
    for(int i=0;i<m_vertices.size();i++) ///Affichage du tableau de booléens pour les composantes fortement connexes
    {
        presence_cfc=false;
        for(int j=0; j<m_vertices.size(); j++)
        {
...
bool* Graph::recherche_cmp(int i,std::map<int,int> indiceForI)
{
    ...
    bool* marquage = new bool[m_vertices.size()]; ///Tableau de booléens qui dit si un sommet a été visité ou non
    bool* pred, *succ; ///Tableau de booléens pour dire si chaque sommet admet un prédecesseur (pred) ou un successeur (succ)
    bool* cmp = new bool[m_vertices.size()]; ///Tableau de booléens pour dire si un sommet fait partie ou non de la composante fortement connexe retournée
    bool suivant=true; ///Booléen pour dire si il reste des sommets à visiter
    pred=new bool[m_vertices.size()]; ///Allocation de mémoire pour les 3 tableaux
            ...
            if(!marquage[j] && pred[j]) ///Si il n'a pas été marqué et qu'il admet un prédecesseur
            {
                ...
                for(int k=0; k<m_vertices.size(); k++) ///Pout tout sommet k du graphe
                {
                    for(unsigned int l=0; l<m_vertices[j].m_vertexout.size(); l++) ///Parmi les successeurs l de j
                    {
                        ...
                        if(k==m_vertices[j].m_vertexout[l]) ///Si k est un successeur de j et qu'il n'a pas été visité
                        {
    ...
    while(suivant) ///Tant que tous les sommets n'ont pas été visités
    {
        ...
        for(int j=0; j<m_vertices.size(); j++) ///Pour tout sommet j du graphe
        {
            if(!marquage[j] && succ[j]) ///Si il n'a pas été marqué et qu'il admet un prédecesseur
            {
                marquage[j]=true; ///Le sommet j a été visité
                for(int k=0; k<m_vertices.size(); k++) ///Pout tout sommet k du graphe
                {
                    for(unsigned int l=0; l<m_vertices[j].m_vertexin.size(); l++) ///Parmi les successeurs l de j
                    {
                        ...
                        if(k==m_vertices[j].m_vertexin[l]) ///Si k est un successeur de j et qu'il n'a pas été visité
                        {
                            succ[k]=true;///On dit que k a un prédecesseur (j)
                            suivant=true; ///Et on fait continuer la boucle
                        }
                    }
